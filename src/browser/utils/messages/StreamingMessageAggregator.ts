...
export class StreamingMessageAggregator {
  ...
  private readonly agentStatusAdapter: AgentStatusAdapter;
  ...
  loadHistoricalMessages(
    ...
    if (!opts?.skipDerivedState) {
      ...
      for (const message of chronologicalMessages) {
        this.skillStore.maybeTrackLoadedSkillFromAgentSkillSnapshot(
          message.metadata?.agentSkillSnapshot
        );
  ...
  getDisplayedMessages(): DisplayedMessage[] {
    if (!this.cache.displayedMessages) {
      ...
      if (initSnapshot) {
        const durationMs =
          initSnapshot.endTime !== null ? initSnapshot.endTime - initSnapshot.startTime : null;
