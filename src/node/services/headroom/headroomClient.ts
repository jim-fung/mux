...
export interface HeadroomStats {
  // Top-level convenience rollups (kept for any legacy callers). The proxy's
  // /stats response is nested; the authoritative fields below are read from
  // their real locations (proxy_inbound / summary.compression / router /
  // persistent_savings) by the router handler.
  total_requests?: number;
  tokens_saved?: number;
  savings_percent?: number;
