...
The image model is configured separately from the chat model. The default is `openai:gpt-image-2`. The first experiment is OpenAI-only and requires OpenAI provider credentials in **Settings → Providers**.
